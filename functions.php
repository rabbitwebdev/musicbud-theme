<?php
/**
 * Functions and definitions musicbudtheme
 *
 * @package musicbudtheme
 */

// Enqueue scripts and styles
function musicbud_theme_enqueue_scripts() {
    // Theme stylesheet
  
    wp_enqueue_style('musicbudtheme-style', get_template_directory_uri() . '/style.css', [], wp_get_theme()->get('Version'));
    ...
    wp_enqueue_script('musicbudtheme-script', get_template_directory_uri() . '/dist/js/app.js', ['jquery'], wp_get_theme()->get('Version'), true);


}
add_action('wp_enqueue_scripts', 'musicbud_theme_enqueue_scripts');

// Theme setup
function musicbud_theme_setup() {
    ...
    register_nav_menus([
        'primary' => __('Primary Menu', 'musicbudtheme'),
         'top_nav' => __('Top Menu', 'musicbudtheme'),
        'footer'  => __('Footer Menu', 'musicbudtheme'),
    ]);

    // Add support for HTML5 markup
    add_theme_support('html5', ['search-form', 'comment-form', 'comment-list', 'gallery', 'caption']);
}
add_action('after_setup_theme', 'musicbud_theme_setup');
...
function musicbud_theme_widgets_init() {
    register_sidebar([
        'name'          => __('Sidebar', 'musicbudtheme'),
        'id'            => 'sidebar-1',
        'description'   => __('Add widgets here.', 'musicbudtheme'),
...
add_action('widgets_init', 'musicbud_theme_widgets_init');


function musicbud_theme_editor_styles() {
    add_editor_style('assets/css/editor-style.css');
}
add_action('admin_init', 'musicbud_theme_editor_styles');
...
function musicbud_theme_custom_excerpt_length($length) {
    return 10; // Adjust the length as needed
}
add_filter('excerpt_length', 'musicbud_theme_custom_excerpt_length');

// Add a custom "Read More" link to excerpts
// function musicbud_theme_excerpt_more($more) {
//     return '<div class="w-100 mt-3 "><a class="link-underline-primary w-100" href="' . get_permalink() . '">' . __('Read More', 'musicbudtheme') . '</a></div>';
// }
// add_filter('excerpt_more', 'musicbud_theme_excerpt_more');

// Load theme's text domain for translations
function musicbud_theme_load_textdomain() {
    load_theme_textdomain('musicbudtheme', get_template_directory() . '/languages');
}
add_action('after_setup_theme', 'musicbud_theme_load_textdomain');

// Disable the WordPress admin bar for all users except administrators
function musicbud_theme_disable_admin_bar() {
    if (!current_user_can('administrator') && !is_admin()) {
        show_admin_bar(false);
    }
}
add_action('after_setup_theme', 'musicbud_theme_disable_admin_bar');

// Cleanup WordPress head
function musicbud_theme_cleanup_head() {
...
add_action('init', 'musicbud_theme_cleanup_head');
